...
bool CConfig::parseCommandLine(int argc, char** argv)
{
	...
	mNTupleCount = MAX_TUPLE_COUNT; // set it again here because this is after we have initialized values from DB.
	bool brc=false;
	char* exeName = argv[0];
	if ((argc > 1) && (argv[1][0]!='?')) 
	{
		while (argc>1)
		{
			...
			if (strncmp(*argv,"-query",6)==0)
			{
				//in query mode, that means we take a file and determine which concept it matches
				QueryMode(true);
				brc = true;
			}
			else if (strncmp(*argv,"-tl",3)==0)
			{
					...
					if ((fpFilelist = P267_open(s, P267_IOREAD))!=NULL)
					{
						...
						while((c = P267_getcf(fpFilelist)) != EOF) 
						{
							...
							else 
							{
								...
								if (!trimmed.empty())
								{
									aURLPage.Set(-1,trimmed,trimmed);

									// Read its first line, which is assumed to be the actual URL
									P267_FILE *fp = P267_open(trimmed.c_str(), P267_IOREAD);
									if (fp != NULL)
									{
										trimmed.erase();
										int c;
										...
										while((c = P267_getcf(fp)) != P267_ENDFILE && (c!=P267_NEWLINE) && (c!=P267_CARRIAGERETURN) && (c!=P267_HTMLDELIMITER)) 
											trimmed += c;
										aURLPage.URL(trimmed);

										if (trimmed.size()> 0)
											mInputFilelist.push_back(aURLPage);
										else
											printf("Discarded file, no HTTP: %s\n", sfilename);
	...
	else if (brc && mInputFilelist.size()>0 && mThreadCount>0)
	{

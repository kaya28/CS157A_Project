...
extern int stem(struct stemmer * z, const char * b, int k)
{
   if (k <= 1)
	   return k; /*-DEPARTURE-*/
...
static char * pBuff;         /* buffer for words tobe stemmed */
...
{  while(TRUE)
   {  int ch = getc(f);
      ...
      {  int i = 0;
         ...
         {  if (i == i_max)
            {  i_max += INC;
               pBuff = (char*) realloc(pBuff, i_max + 1);
            }

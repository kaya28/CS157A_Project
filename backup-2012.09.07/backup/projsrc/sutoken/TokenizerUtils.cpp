...
bool isNumber(const char* ss)
{
	...
	try 
	{
		...
		while (len>0)
		{
			if (ss[len-1]>=0 && !isdigit(ss[len-1]))
			{
				bResult=false;
				break;
			}
	...
	{
		printf("*** Exception in isNumber(%s)\n", ss);
	}
...
//Set of rules to determine if this is the type of token we should keep or discard.
//Return true if keep.
//This is different that isStopWord because it is called AFTER the token has been identified and is ready to be added to list.
bool keepToken(const string & pToken)
{
	return false;
}
